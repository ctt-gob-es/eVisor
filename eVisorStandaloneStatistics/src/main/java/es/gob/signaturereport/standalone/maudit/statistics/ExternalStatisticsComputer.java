 ...
 */
package es.gob.signaturereport.standalone.maudit.statistics;
...
import es.gob.signaturereport.standalone.maudit.statistics.access.StandaloneStatisticPersistenceException;
import es.gob.signaturereport.standalone.maudit.statistics.access.StandaloneStatisticPersistenceFacade;
...
public class ExternalStatisticsComputer {
	...
	public static void main(String[ ] args) {
		...
		if(args==null || args.length==0){
			...
			try {
				...
				LOGGER.error(LanguageStandalone.getFormatResStandaloneStatistics(LanguageKeys.AUD_048, new Object[]{args[0]}));
			}
		}
	...
	private void init(){
		...
		try {
			...
			LOGGER.error(LanguageStandalone.getFormatResStandaloneStatistics(LanguageKeys.AUD_049, new Object[]{COMPUTER_PROPERTIES}));
		}finally{
	...
	public void computeStatistics(){
		if(this.initialized){
			try {
				StandaloneStatisticPersistenceFacade.getInstance().computeStatistics(calculationDate, updateValues, false);
			} catch (StandaloneStatisticPersistenceException e) {
				LOGGER.error(LanguageStandalone.getFormatResStandaloneStatistics(LanguageKeys.AUD_051, new Object[]{calculationDate.toString()}));
				if(sendAlarm){
	...
	private void communicateAlarm(String msg) {
		...
		mail.setSubject(LanguageStandalone.getResStandaloneStatistics(LanguageKeys.AUD_052));
