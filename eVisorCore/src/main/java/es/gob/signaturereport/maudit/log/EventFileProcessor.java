...
public class EventFileProcessor {
		...
		try {
			...
			if (filePath != null) {
				...
				if ((auditFiles != null) && (auditFiles.length == 1)){
					...
					LOGGER.error(Language.getFormatMessage(LanguageKeys.AUD_032, new Object[ ] { filePath }));
				}
			}
			...
			LOGGER.error(Language.getMessage(LanguageKeys.AUD_033), e);
		} catch (NumberFormatException nfe) {
			LOGGER.error(Language.getMessage(LanguageKeys.AUD_034), nfe);
		}
	}
	...
	public void readAndSave() {
		LOGGER.info(Language.getFormatMessage(LanguageKeys.AUD_031, new Object[ ] { this.eventFilePath }));
		...
		LOGGER.info(Language.getFormatMessage(LanguageKeys.AUD_035, new Object[ ] { Long.valueOf(time) }));
	}
	...
	private boolean storeEventFile() {
		try {
			boolean stored = AuditPersistenceFacade.getInstance().existEventFile(this.eventFileId, false);
			if (!(stored)){
				AuditPersistenceFacade.getInstance().storeEventFile(this.eventFileId, this.custodyMode, this.creationTime, null, false);
			}
		} catch (AuditPersistenceException e) {
			String message = Language.getMessage(LanguageKeys.AUD_036);
	...
	private boolean saveAuditTraces() {
		...
		try {
				...
				if (trace != null) {
					...
					if (trace.getActionId() == AuditManagerI.OPEN_TRANSACTION_ACT) {
						...
						if (i >= NUM_TRANSACTIONS) {
							try {
								AuditPersistenceFacade.getInstance().saveCompletedTraces(this.eventFileId, closedTransactions, false);
							} catch (AuditPersistenceException e) {
								String message = Language.getMessage(LanguageKeys.AUD_038);
								LOGGER.error(message, e);
								if (this.sendAlarm) {
									communicateAlarm(message);
								}
			...
			if (!(transactions.isEmpty())) {
				try {
					AuditPersistenceFacade.getInstance().saveCompletedTraces(this.eventFileId, transactions, false);
				} catch (AuditPersistenceException e) {
					String message = Language.getMessage(LanguageKeys.AUD_038);
					LOGGER.error(message, e);
					if (this.sendAlarm) {
			...
			if (!(openedTransactions.isEmpty())) {
				try {
					AuditPersistenceFacade.getInstance().saveOpenedTraces(openedTransactions, false);
				} catch (AuditPersistenceException e) {
					String message = Language.getMessage(LanguageKeys.AUD_038);
					LOGGER.error(message, e);
					if (this.sendAlarm) {
			...
			String message = Language.getMessage(LanguageKeys.AUD_039);
			LOGGER.error(message, e);
			if (this.sendAlarm){
				communicateAlarm(message);
			}
	...
	private void storeContent() {
		...
		try {
			...
			if (hash != null){
				AuditPersistenceFacade.getInstance().updateEventContent(this.eventFileId, hash, this.custodyMode);
			}else{
				AuditPersistenceFacade.getInstance().updateEventContent(this.eventFileId, file, this.custodyMode);
			}
		} catch (ToolsException e) {
			message = Language.getMessage(LanguageKeys.AUD_039);
			LOGGER.error(message, e);
			if (this.sendAlarm){
				communicateAlarm(message);
			}
		} catch (NoSuchAlgorithmException e) {
			message = Language.getFormatMessage(LanguageKeys.AUD_027, new Object[ ] { this.eventFilePath });
			LOGGER.error(message, e);
			if (this.sendAlarm){
				communicateAlarm(message);
			}
		} catch (AuditPersistenceException e) {
			message = Language.getMessage(LanguageKeys.AUD_040);
			LOGGER.error(message, e);
			if (this.sendAlarm){
				communicateAlarm(message);
			}
	...
	private void communicateAlarm(String msg) {
		...
		mail.setSubject(Language.getMessage(LanguageKeys.AUD_037));
